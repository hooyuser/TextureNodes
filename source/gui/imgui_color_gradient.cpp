...
{
	if (m_marks.size() == max_size) {
		return;
	}
	auto mark = ImGradientMark{
		.color = {color.Value.x, color.Value.y, color.Value.z},
		.position = ImClamp(position, 0.0f, 1.0f),
	};
	auto pos = std::upper_bound(m_marks.begin(), m_marks.end(), mark);
  
	m_marks.insert(pos, mark);
...
{
	auto position = std::find(m_marks.begin(), m_marks.end(), mark);
	if (position != m_marks.end()) {// == myVector.end() means the element was not found
		m_marks.erase(position);
	}
...
void ImGradient::computeColorAt(float position, float* color)
{
	position = ImClamp(position, 0.0f, 1.0f);
